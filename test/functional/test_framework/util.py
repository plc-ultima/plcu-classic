...
COIN = 100000000 # 1 PLCUC in satoshis
BASE_CB_AMOUNT = Decimal(5000)
CB_AMOUNT_AFTER_BLOCK_100 = Decimal('0.005')
CB_AMOUNT_AFTER_REDUCTION = Decimal('0.00005000')
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s PLCUC too low! (Should be %s PLCUC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s PLCUC too high! (Should be %s PLCUC)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting PLCUC values"""
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "plcultimac.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "plcultimac.conf")):
        with open(os.path.join(datadir, "plcultimac.conf"), 'r', encoding='utf8') as f:
            for line in f:
...
def verify_tx_sent(node, txid):
    ...
    assert_greater_than_or_equal(MAX_FEE, fee)
...
def show_time(name):
    global last_times
    if name not in last_times:
        last_times[name] = 0
    last_time = last_times[name]
    now = time.time()
    dur = (now - last_time) if last_time else 0
    last_times[name] = now
    return f'{dur:.2f}'
