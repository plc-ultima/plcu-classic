...
class BitcoinTestFramework(object):
    """Base class for a plcultimac test script.

    Individual plcultimac test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave plcultimacds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop plcultimacds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing plcultimacd/plcultimac-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: plcultimacds were not stopped and may still be running")
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'plcultimacd exited' in str(e), f'another error got: {e}'  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "plcultimacd should have exited with an error"
                else:
                    assert_msg = "plcultimacd should have exited with expected error " + expected_msg
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("PLCULTIMACD", "plcultimacd"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in plcultimac.conf
    ...
    def _initialize_chain_from_parent(self):
        ...
        assert height > 200, f'invalid height: {height}'
        create_cache = False
        for i in range(MAX_NODES):
            if not os.path.isdir(os.path.join(self.options.cachedir, 'node' + str(i))):
                create_cache = True
        ...
        if create_cache:
            ...
            if self.options.parentcache:
                ...
                for i in range(MAX_NODES):
                    ...
                    shutil.copytree(from_dir, to_dir)
                    initialize_datadir(self.options.cachedir, i)  # Overwrite port/rpcport in plcultimac.conf
            else:
            ...
            # Create cache directories, run plcultimacds:
            for i in range(MAX_NODES):
            ...
            self.log.debug(f'nodes started, actual height: {blockcount}')
            blocks_left = height - blockcount
            while blocks_left > 0:
                for peer in range(4):
                    next_portion = min(25, blocks_left)
                    if next_portion:
                        self.nodes[peer].generate(next_portion)
                        blocks_left -= next_portion
                        # Must sync before next peer starts generating blocks
                        sync_blocks(self.nodes)
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    Sets up some plcultimacd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("PLCULTIMACD", "plcultimacd"),
                          help="plcultimacd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("PLCULTIMACD", "plcultimacd"),
                          help="plcultimacd binary to use for reference nodes (if any)")
