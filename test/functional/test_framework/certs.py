...
def pubkey_bin_from_priv_key_base58(priv_key_base58):
    key = CECKey()
    key.set_secretbytes(Base58ToSecretBytes(priv_key_base58))
...
def send_block(node, test_node, block, accepted, reject_reasons=[], verbose=True):
    bestblockhash_before = node.getbestblockhash()
    ...
    if not accepted and reject_reasons:
        assert_in(test_node.reject_message.reason.decode('ascii'), reject_reasons)
    test_node.reject_message = None
...
def generate_outpoints(node, count, amount, address):
    ...
    for i in range(count):
        ...
        outpoints.append(COutPoint(int(txid, 16), n))
        fee_sum += node.gettransaction(txid)['fee']
        logger.debug(f'\t{txid}:{n}')
...
def generate_certs_pair(node, test_node, root_cert_key=None, root_cert_flags=None, root_cert_hash=None,
                        ...
                        pass_cert_revoked=False, super_key=None, keep_super_key=None, fee=Decimal('0.00001000'),
                        pass_cert_flag_default=0):
    # Root cert:
    ...
    (tx2, super_key1) = compose_cert_tx(outpoints.pop(0), Decimal(1), pass_cert_key, pass_cert_name,
                                        pass_cert_flags, keep_super_key, block1_hash=pass_cert_sig_hash,
                                        block2a=pass_cert_signature, parent_key_for_block2=pass_cert_sig_key)
    ...
    if pass_cert_revoked:
        ...
        (tx2a, _) = compose_cert_tx(COutPoint(int(pass_cert_hash, 16), 0), Decimal('0.9'), pass_cert_key,
                                    pass_cert_name, pass_cert_flags, keep_super_key, prev_scriptpubkey=prev_scriptpubkey)
        send_tx(node, test_node, tx2a, True)
...
def restart_node_with_cert(test_fr, use_cert, super_key_pubkey=None, root_cert_hash=None, pass_cert_hash=None,
    ...
    if gen_block:
        node1 = test_fr.nodes[next_indexes[0] if next_indexes else index]
        node1.generate(1)
        sync_blocks(nodes_to_sync)


def parse_moneybox_utxos(node, block_hash=None, verbose=False):
    moneybox_utxos = []
    moneybox_utxo_amounts = {}
    block_hash = block_hash if block_hash else node.getbestblockhash()
    block = node.getblock(block_hash, 1)
    txid_cb = block['tx'][0]
    tx_cb = node.getrawtransaction(txid_cb, 1)
    for v in tx_cb['vout']:
        if v['scriptPubKey']['type'] == 'scripthash' and v['scriptPubKey']['addresses'][0] == MoneyboxP2SHAddress():
            ...
            moneybox_utxo_amounts[outpoint] = v['value']
    if verbose:
        logger.debug(f'tx_cb: {tx_cb}')
    return (moneybox_utxos, moneybox_utxo_amounts)
