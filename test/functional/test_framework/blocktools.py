...
fee_reduction_height = {
    'main': 25000,
    'testnet': 119000,
    'regtest': 2000,
}
fee_quartering_height = {
    'main': 0xFFFFFFFF,
    'testnet': 0xFFFFFFFF,
    'regtest': 2500,
}
...
def get_subsidy(height, minerfees, network='regtest', fee_reduction_block=None, fee_quartering_block=None):
    fee_reduction_block = fee_reduction_block if fee_reduction_block is not None else fee_reduction_height[network]
    fee_quartering_block = fee_quartering_block if fee_quartering_block is not None else fee_quartering_height[network]
    if height <= 100:
        return ToSatoshi(BASE_CB_AMOUNT)
    elif height <= fee_reduction_block:
        return max(ToSatoshi(CB_AMOUNT_AFTER_BLOCK_100), int(minerfees / 2))  # int(0.005*COIN)
    elif height <= fee_quartering_block:
        return max(ToSatoshi(CB_AMOUNT_AFTER_REDUCTION), int(minerfees / 2))  # int(0.00005*COIN)
    return max(ToSatoshi(CB_AMOUNT_AFTER_REDUCTION), int(minerfees / 4))  # int(0.00005*COIN)
...
def create_coinbase(height, pubkey=None, minerfees=0, refill_moneybox_amount=0, granularity=None,
                    moneyboxscript=GetP2SHMoneyboxScript(), total_bc_amount=None, network='regtest',
                    fee_reduction_block=None, fee_quartering_block=None, force_add_mausoleum_output=None):
    fee_quartering_block = fee_quartering_block if fee_quartering_block is not None else fee_quartering_height[network]
    ...
    subsidy = CTxOut()
    subsidy.nValue = get_subsidy(height, minerfees, network=network, fee_reduction_block=fee_reduction_block,
    ...
    if (subsidy.nValue > 0):
        ...
        need_add_mausoleum_output = False
        if force_add_mausoleum_output is None:
            # regular workflow: auto
            if height > fee_quartering_block:
                need_add_mausoleum_output = True
        elif force_add_mausoleum_output:
            need_add_mausoleum_output = True
        if need_add_mausoleum_output and minerfees > subsidy.nValue:
            mausoleum_output = CTxOut()
            mausoleum_output.nValue = minerfees - subsidy.nValue
            mausoleum_output.scriptPubKey = GraveScript2()
            coinbase.vout.append(mausoleum_output)
