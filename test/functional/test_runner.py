...
def main():
    ...
    parser.add_argument('--testsuitename', '-n', default='PLC Ultima Classic Node functional tests',
    ...
    parser.add_argument('--skipmorecache', action='store_true',
                        help='the default behavior is to create all caches. --skipmorecache will only create the base cache and skip all others.')
    ...
    tmpdir = "%s/plcultimac_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and plcultimacd must all be enabled")
    ...
    run_tests(test_list, config["environment"]["SRCDIR"], config["environment"]["BUILDDIR"],
              config["environment"]["EXEEXT"], args.junitoutput, tmpdir, args.jobs, args.testsuitename, args.coverage,
              passon_args, start_point, args.timeout, args.skipmorecache)


def run_tests(test_list, src_dir, build_dir, exeext, junitoutput, tmpdir, jobs=1, test_suite_name="PLCUC",
              enable_coverage=False, args=[], start_point=0, timeout=0, skipmorecache=False):
    # Warn if plcultimacd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "plcultimacd"]) is not None:
            print("%sWARNING!%s There is already a plcultimacd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "PLCULTIMACD" not in os.environ:
        os.environ["PLCULTIMACD"] = build_dir + '/src/plcultimacd' + exeext
        os.environ["PLCULTIMACCLI"] = build_dir + '/src/plcultimac-cli' + exeext
    ...
    if len(test_list) > 1 and jobs > 1:
        ...
        if not skipmorecache:
            ...
            for value in [400, 495, 500, 1300, 1350, 1400, 1500]:
...
class RPCCoverage(object):
    ...
    commands per `plcultimac-cli help` (`rpc_interface.txt`).
