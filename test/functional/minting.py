#!/usr/bin/env python3
# Copyright (c) 2019 The PLC Ultima Classic Core developers
...
class MintingTest(BitcoinTestFramework):
    ...
    def parse_moneybox_inputs_from_block(self, node, blockhash=None, sort=True, verbose=True):
        moneybox_scriptpubkey = bytes_to_hex_str(GetP2SHMoneyboxScript())
        if not blockhash:
        ...
        for i, tx in enumerate(block['tx']):
            ...
            for output in tx['vout']:
                if output['scriptPubKey']['hex'] == moneybox_scriptpubkey:
                    n = output['n']
                    value = output['value']
                    scriptpubkey = output['scriptPubKey']['hex']
                    if i == 0:  # coinbase tx
                        self.moneybox_utxos_immature.append((txid, n, value, scriptpubkey, height + 100))
                    else:
                        self.moneybox_utxos.append((txid, n, value, scriptpubkey))
                    if verbose:
                        self.log.debug(f'moneybox_utxo parsed: {txid}:{n}, {value}, {scriptpubkey}, {i}')

        # Move mature utxos to self.moneybox_utxos:
        to_del = []
        for u in self.moneybox_utxos_immature:
            if u[4] <= height:
                self.moneybox_utxos.append((u[0], u[1], u[2], u[3]))
                to_del.append(u)
                self.log.debug(f'moneybox_utxo matured: {u[0]}:{u[1]}')
        for dd in to_del:
            self.moneybox_utxos_immature.remove(dd)

        if sort:
            self.moneybox_utxos = sorted(self.moneybox_utxos, key=lambda e: e[2])
    ...
    def moneybox_inputs_from_cache(self):
        ...
        while os.path.isfile(moneybox_inputs_lock):
            self.log.debug(f'waiting for {moneybox_inputs_lock} ...')
            time.sleep(1)

        if os.path.isfile(moneybox_inputs_json):
            self.log.debug(f'loading moneybox_inputs from {moneybox_inputs_json} ...')
            moneybox_inputs = json.load(open(moneybox_inputs_json))
            self.moneybox_utxos = [(i[0], i[1], Decimal(i[2]), i[3]) for i in moneybox_inputs]
            self.log.debug(f'loaded {len(self.moneybox_utxos)} moneybox_inputs')
        else:
            os.makedirs(minting_dir, exist_ok=True)
            with FileLock(moneybox_inputs_lock):
                self.log.debug('will parse moneybox_inputs...')
                node0 = self.nodes[0]
                height = node0.getblockcount()
                for i in range(1, height + 1):
                    hash = node0.getblockhash(i)
                    self.parse_moneybox_inputs_from_block(node0, hash, i == height, False)
                moneybox_inputs_copy = [(i[0], i[1], str(i[2]), i[3]) for i in self.moneybox_utxos]
                with open(moneybox_inputs_json, 'w') as file:
                    file.write(json.dumps(moneybox_inputs_copy, indent=2))
                self.log.debug(f'parsed {len(self.moneybox_utxos)} moneybox_inputs and wrote them to {moneybox_inputs_json}')

        assert_equal(len(self.moneybox_utxos), 1000)
    ...
    def mint(self, user_inputs, utxo_cert_root, utxo_cert_ca3, params, more_moneybox_utxos=[]):
        node0 = self.nodes[0]
        ...
        assert_greater_than(len(self.moneybox_utxos) + len(more_moneybox_utxos), 0)
        attemps = 0
        ...
        try:
            ...
            while True:
                ...
                for moneybox_input in self.moneybox_utxos + more_moneybox_utxos:
                    moneybox_input_txid = moneybox_input[0]
                    moneybox_input_vout = moneybox_input[1]
                    moneybox_input_amount = moneybox_input[2]
                    moneybox_input_scriptpubkey = moneybox_input[3]
                    print_to_buffer('tx3 vin[{}] moneybox_input: {}'.format(len(tx3.vin), moneybox_input_amount))
                    seq = 0xfffffffe if self.locked_outputs else 0xffffffff
                    tx3.vin.append(CTxIn(COutPoint(int(moneybox_input_txid, 16), moneybox_input_vout),
                                         hex_str_to_bytes(moneybox_input_scriptpubkey), seq))
                    reward_taken += moneybox_input_amount
                    moneybox_inputs_taken += 1
                ...
                if not moneybox_inputs_enough:
                    moneybox_sum = sum(elem[2] for elem in self.moneybox_utxos)
                    more_moneybox_sum = sum(elem[2] for elem in more_moneybox_utxos)
                    print_to_buffer('moneybox_inputs: {}'.format(self.moneybox_utxos))
                    print_to_buffer('more_moneybox_inputs: {}'.format(more_moneybox_utxos))
                    print_to_buffer(f'Alarm! Not enough moneybox money! moneybox_inputs: {len(self.moneybox_utxos)}, '
                                    f'moneybox_sum: {moneybox_sum}, more_moneybox_inputs: {len(more_moneybox_utxos)}, '
                                    f'more_moneybox_sum: {more_moneybox_sum}, reward_to_ben: {reward_to_ben}, '
                                    f'reward_taken: {reward_taken}, height: {node0.getblockcount()}')
                    assert_equal(moneybox_inputs_enough, True)

                relayfee = node0.getnetworkinfo()['relayfee']  #  (numeric) minimum relay fee for non-free transactions in PLCUC/kB
                HASH160_SIZE = 20
                P2PKH_OUTPUT_BYTES = (
                        8 +   # nValue
                        1 +   # scriptPubkeyLen
                ...
                self.log.debug(f'user_output_bytes: {user_output_bytes}, user_inputs_lens_sum: {user_inputs_lens_sum}, '
                               f'moneybox_inputs_lens_sum: {moneybox_inputs_lens_sum}, '
                               f'bytes_user_part: {bytes_user_part}, bytes_moneybox_part: {bytes_moneybox_part}, '
                               f'COMMON_BYTES: {COMMON_BYTES}')
                ...
                print_to_buffer(f'mint: fee calculation: relayfee: {relayfee}, bytes_user_part: {bytes_user_part}, '
                                f'bytes_moneybox_part: {bytes_moneybox_part}, total_fee_calculated: {total_fee_calculated}, '
                                f'total_fee: {total_fee}, fee_user_percent: {fee_user_percent}')
                ...
                if len(self.get_real_reward_outputs_names(reward_to, params)) > 0:
                    ...
                    print_to_buffer(f'reward_payed: {ToCoins(reward_payed2)}, reward_to_ben: {reward_to_ben}, '
                                    f'fee_user: {ToCoins(fee_user)}, burn: {burn}')
                if reward_payed2 > 0:
                ...
                if fee_moneybox > reward_change:
                    ...
                    print_to_buffer(f'need more moneybox inputs, will continue, fee_moneybox: {fee_moneybox}, '
                                    f'reward_change: {reward_change}, reward_taken: {reward_taken}, '
                                    f'more_inputs_count: {more_inputs_count}')
                    continue
                ...
                if reward_change > ToCoins(DUST_OUTPUT_THRESHOLD) or (reward_change > 0 and not drop_moneybox_dust_change):
                    print_to_buffer(f'moneybox change (reward_change_orig - fee_moneybox): {ToCoins(reward_change)}, '
                                    f'reward_change_orig: {reward_change_orig}, fee_moneybox: {ToCoins(fee_moneybox)}')
                ...
                # use temp tx copy (tx3_tmp_copy) to pass it as a parameter to optimize performance,
                # otherwise temp copy will be created inside SignatureHash function with deep copy on each iteration,
                # that will be slow on big number of inputs:
                tx3_tmp_copy = CTransaction(tx3)
                for txin in tx3_tmp_copy.vin:
                    txin.scriptSig = b''

                for i in range(len(user_inputs)):
                    (sig_hash, err) = SignatureHash(CScript(tx3.vin[i].scriptSig), tx3, i, SIGHASH_ALL, tx3_tmp_copy)
                    assert (err is None)
                ...
                for i in range(len(user_inputs), len(tx3.vin) - int(self.use_taxfree_cert)):
                    ...
                    (sig_hash, err) = SignatureHash(scriptSig, tx3, i, SIGHASH_ALL, tx3_tmp_copy)
                    assert (err is None)
            ...
            for _ in range(moneybox_inputs_taken):
                moneybox_inputs = self.moneybox_utxos if self.moneybox_utxos else more_moneybox_utxos
                moneybox_inputs.pop(0)
    ...
    def create_root_certificate(self, params):
        ...
        # Скрипт переводит с адреса ROOT_PKH на этот же адрес некоторую сумму, например 1 PLCUC, активируя CA3_PKH ключ.
        (ca3_keys, _, _, utxo_cert_root, _) = self.create_cert(utxo_coins_root, rootcertamount,
    ...
    def run_testcase(self, params):
        ...
        if params['accepted']:
            ...
            self.parse_moneybox_inputs_from_block(node0)
    ...
    def run_special_use_immature_moneybox(self, params, args):
        ...
        utxo_cert_ca3 = args['utxo_cert_ca3']
        name = params['name']
        immature_moneybox_mempool = (name == 'special_use_immature_moneybox_mempool')
        change_4coins = ((4 * COIN) if immature_moneybox_mempool else 0)
        accepted_orig = params['accepted']
        assert_greater_than(len(user_outputs), 2)
        last_user_output = user_outputs.pop()
        MONEYBOX_AMOUNT = 100000 * COIN
        assert_equal(MONEYBOX_AMOUNT % len(user_outputs), 0)
        reward_exhaust_moneybox = MONEYBOX_AMOUNT // len(user_outputs)
        assert_greater_than(reward_exhaust_moneybox, change_4coins)
        spent_total = 0

        # spend entire moneybox in several iterations, leave change_4coins as change in the last one:
        for i, out in enumerate(user_outputs):
            ...
            params['rewardamount'] = reward_exhaust_moneybox - change_4coins if last_iteration else reward_exhaust_moneybox
            params['accepted'] = True
            (txid, _, spent_now, _) = self.mint([out], utxo_cert_root, utxo_cert_ca3, params)
            spent_total += spent_now

        # last step: try to mint using immature moneybox inputs:
        self.log.info('Will run the last step...')

        if immature_moneybox_mempool:
            # Now moneybox is exhausted, except the single output with change 4 coins,
            # which is in the mempool:
            tx = node0.getrawtransaction(txid, 1)
            moneybox_out_n = find_output_by_address(node0, MoneyboxP2SHAddress(), None, tx)
            output = tx['vout'][moneybox_out_n]
            value = output['value']
            scriptpubkey = output['scriptPubKey']['hex']
            immature_moneybox_inputs = [(txid, moneybox_out_n, value, scriptpubkey)]
        else:
            # Now moneybox is exhausted, we have only moneybox_utxos_immature
            assert_equal(len(self.moneybox_utxos), 0)
            self.refill_moneybox(spent_total, params)
            self.parse_moneybox_inputs_from_block(node0)
            immature_moneybox_inputs = self.moneybox_utxos_immature

        assert_equal(len(self.moneybox_utxos), 0)
        assert_greater_than_or_equal(len(immature_moneybox_inputs), 1)
        self.log.debug(f'immature_moneybox_inputs ({len(immature_moneybox_inputs)}): {immature_moneybox_inputs[:1]}')

        params['rewardamount'] = 4 * COIN
        params['accepted'] = accepted_orig
        (_, _, _, _) = self.mint([last_user_output], utxo_cert_root, utxo_cert_ca3, params, immature_moneybox_inputs)
