...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated plcultimac to.\n"
...
// NOTE: Unlike wallet RPC (which use PLCUC values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PLC Ultima Classic is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PLC Ultima Classic is downloading blocks...");
    ...
    if (pblock->vtx[0]->vout.size() >= 2)
    {
        ...
        for (size_t i = 0; i < pblock->vtx[0]->vout.size(); ++i)
        {
            ...
            if (out.scriptPubKey != Params().mausoleumScriptPubKey() &&
                    out.scriptPubKey != Params().moneyBoxScriptPubKey())
            {
                // simple value
                values.push_back(static_cast<int64_t>(out.nValue));
            }
            else
            {
                // object with address
                // Params().mausoleumScriptPubKey()
                // Params().moneyBoxScriptPubKey()
                CTxDestination dest;
                ExtractDestination(out.scriptPubKey, dest);
                UniValue v(UniValue::VOBJ);
                v.push_back(Pair(CBitcoinAddress(dest).ToString(), static_cast<int64_t>(out.nValue)));
                values.push_back(v);
            }
        }
...
UniValue estimaterawfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"feerate\" : x.x,        (numeric, optional) estimate fee-per-kilobyte (in PLCUC)\n"
