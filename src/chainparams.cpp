...
CScript CChainParams::moneyBoxScriptPubKey()
{
    CScript scr;
    scr << OP_CHECKREWARD;
    CScriptID id(scr);
...
CScript graveScriptPubKey(const std::string & graveAddress, const CChainParams & params)
{
    return GetScriptForDestination(CBitcoinAddress(graveAddress).Get(params));
}

CScript CChainParams::mausoleumScriptPubKey()
{
    CScript scr;
...
public:
    CMainParams() {
        ...
        consensus.moneyBoxAddress = moneyBoxScriptPubKey();
        consensus.graveAddresses  = {std::make_pair(graveScriptPubKey("U2xHJx3f6hbaDW4FvFvANLL4FJhuxg5Bo12ho", *this), 0.02),
                                     std::make_pair(mausoleumScriptPubKey(), 0.01)};

        consensus.maxCaBlock                 = 100000;
        consensus.defaultFeeReductionBlock   = 35000;
        consensus.defaultFeeQuarteringBlock  = 386792;
...
public:
    CTestNetParams() {
        ...
        consensus.moneyBoxAddress = moneyBoxScriptPubKey();
        consensus.graveAddresses  = {std::make_pair(graveScriptPubKey("U2xFeMxJfqbjGFEoCiQ3wFProGrDct9Ep7Snk", *this), 0.02),
                                     std::make_pair(mausoleumScriptPubKey(), 0.01)};

        consensus.maxCaBlock                 = 0;
        consensus.defaultFeeReductionBlock   = 29000;
        consensus.defaultFeeQuarteringBlock  = 431000;
...
public:
    CRegTestParams() {
        ...
        consensus.defaultFeeQuarteringBlock  = 2500;
    ...
    void init()
    {
        ...
        std::string gran_pair = gArgs.GetArg("-moregranularity-regtest", "");
        auto delim_index = gran_pair.find(':');
        if (delim_index != std::string::npos)
        {
            std::string token1 = gran_pair.substr(0, delim_index);
            std::string token2 = gran_pair.substr(delim_index + 1);
            auto val1 = stoll(token1);
            auto val2 = stoll(token2);
            if (val1 && val2 && val1 > consensus.granularities.back().first)
            {
                consensus.granularities.push_back(std::make_pair(val1, val2));
                std::string gran_str;
                for (const auto& e : consensus.granularities)
                {
                    gran_str += ("(" + std::to_string(e.first) + ": " + std::to_string(e.second) + "), ");
                }
                LogPrintf("Updated granularities: %s\n", gran_str);
            }
            else
            {
                LogPrintf("Ignore parameter -moregranularity-regtest, invalid value\n");
            }
        }
    }
...
//******************************************************************************
//******************************************************************************
int64_t CChainParams::maxPossibleGranularity() const
{
    static int64_t gran = [](const std::vector<std::pair<uint32_t, int64_t> > & granularities){
        int64_t maxg = 0;
        for (const std::pair<uint32_t, int64_t> & g : granularities)
        {
            maxg = std::max(g.second, maxg);
        }
        return maxg;
    }(consensus.granularities);
    return gran;
}
...
//******************************************************************************
//******************************************************************************
int CChainParams::feeReductionBlock() const
{
    if (NetworkIDString() == CBaseChainParams::REGTEST)
    {
        return gArgs.GetArg("-feereductionblock-regtest", consensus.defaultFeeReductionBlock);
    }
    return consensus.defaultFeeReductionBlock;
}

//******************************************************************************
//******************************************************************************
int CChainParams::feeQuarteringBlock() const
{
    if (NetworkIDString() == CBaseChainParams::REGTEST)
    {
        return gArgs.GetArg("-feequarteringblock-regtest", consensus.defaultFeeQuarteringBlock);
    }
    return consensus.defaultFeeQuarteringBlock;
}
