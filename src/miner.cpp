                                                               ...
                                                               bool fMineWitnessTx)
{
    ...
    // miner subsidy
    std::pair<CAmount, bool> subsidy = GetBlockSubsidy(nHeight, nFees, chainparams);
    ...
    {
        // rest of fee to mausoleum
        CAmount restOfFee = (nFees > subsidy.first) ? (nFees - subsidy.first) : 0;
        if (restOfFee > 0)
        {
            coinbaseTx.vout.push_back(CTxOut(restOfFee, chainparams.mausoleumScriptPubKey()));
        }
    }
