...
#if defined(NDEBUG)
# error "PLC Ultima Classic cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "PLC Ultima Classic Signed Message:\n";
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    std::pair<CAmount, bool> blockSubsidy = GetBlockSubsidy(pindex->nHeight, nFees, chainparams);

    CAmount foundSubsidy  = 0;
    CAmount foundEmbalmed = 0;
    ...
    for (const CTxOut & out : block.vtx[0]->vout)
    {
        ...
        if (out.scriptPubKey == chainparams.moneyBoxAddress())
        {
            ...
            {
                return state.DoS(100,
                                 error("ConnectBlock(): invalid PLCUC granularity in coinbase"),
                                       REJECT_INVALID, "bad-cb-amount");
            }
        ...
        else if (out.scriptPubKey == chainparams.mausoleumScriptPubKey())
        {
            foundEmbalmed += out.nValue;
        }
        else
        {
    ...
    if (pindex->nHeight > chainparams.feeQuarteringBlock())
    {
        CAmount neededEmbalmed = (nFees > blockSubsidy.first) ? (nFees - blockSubsidy.first) : 0;
        if (foundEmbalmed != neededEmbalmed)
        {
            return state.DoS(100,
                             error("ConnectBlock(): invalid burmed value in coinbase (actual=%d vs required=%d)",
                                   foundEmbalmed, neededEmbalmed),
                                   REJECT_INVALID, "bad-cb-burned");
        }
    }
    ...
    if (pindex->nHeight <= chainparams.GetConsensus().countOfInitialAwardBlocks)
    {
        ...
        {
            return state.DoS(100,
                             error("ConnectBlock(): invalid PLCUC award value in coinbase (in initial blocks)",
    ...
    {
        return state.DoS(100,
                         error("ConnectBlock(): invalid PLCUC award value in coinbase (actual=%d vs limit=%d, height=%d, total=%d)",
